...
def pre_process_cycle(DG, nodes_in_cycle, nodes_in_cycle_set, factors_in_cycle, factors_in_cycle_set):
    ...
    if DG.nodes[nodes_in_cycle[1]]['n_parent'] + DG.nodes[nodes_in_cycle[1]]['n_child'] <= 2:
        ...
        startAnchorNode_factors_nextAnchorNode[start_anchor_node]["next_outIteraction_factors"] = list(
            factor_parentNode_childNode_outsideCycle.keys())
        ...
        if len(startAnchorNode_factors_nextAnchorNode.keys()) == 1:
            ...
            if len(factor_parentNode_childNode_outsideCycle) == 0:
                ...
                inCycle_parentFactor = \
                    list(filter(lambda x: x in factors_in_cycle_set, list(DG.predecessors(start_anchor_node)))).copy()[
                        0]
...
class MPO:
    ...
    def _init_belif(self):
        for factor_i in self.factors:
            ...
            if len(child_nodes) == 1 and len(parent_nodes) > 1:
                sum_parent_nodes_belief = float(sum(self.belief[parent_nodes].values[0]))
    ...
    def _init_msg_nodes2factors(self):
        for node_i, parent_child in self.nodes.items():
            ...
            for parent_factor_i in parent_factors:
                ...
                self._msg_nodes2factors[(node_i, parent_factor_i)] = float(self.belief[node_i].values[0])
            ...
            for child_factor_i in child_factors:
                ...
                self._msg_nodes2factors[(node_i, child_factor_i)] = float(self.belief[node_i].values[0])
    ...
    def update_msg_factor2node(self, factor, cur_node, parent_nodes, child_nodes, flag):
        ...
        if msg_from_parent_nodes == 0:
            ...
            msg_factor2node = -msg_factor2node
        elif msg_from_child_nodes != 0 and msg_from_parent_nodes != 0 and msg_factor2node > 0 and flag == 'parent_node':
            msg_factor2node = msg_factor2node
        elif msg_from_child_nodes != 0 and msg_from_parent_nodes != 0 and msg_factor2node > 0 and flag == 'child_node':
            # msg_factor2node is < 0
            # msg_factor2node = -msg_factor2node
            msg_factor2node = ((1.0 - negative_keep_ratio) * self._msg_nodes2factors[
                (cur_node, factor)] + negative_keep_ratio * (-msg_factor2node))
        elif msg_from_child_nodes != 0 and msg_from_parent_nodes != 0 and msg_factor2node < 0 and flag == 'parent_node':
            # msg_factor2node is < 0
            # msg_factor2node = msg_factor2node
            msg_factor2node = ((1.0 - negative_keep_ratio) * self._msg_nodes2factors[
    ...
    def update_msg_factor2nodes(self, factor, parent_nodes, child_nodes):
        ...
        for parent_node_i in parent_nodes:
            self.update_msg_factor2node(factor, parent_node_i, parent_nodes, child_nodes, flag='parent_node')

        for child_node_i in child_nodes:
            self.update_msg_factor2node(factor, child_node_i, parent_nodes, child_nodes, flag='child_node')
    ...
    def get_weights_4_msg(self, factors):
        ...
        if n_factors_in_main_branch == 0:
            factors_weights = {factor_i: 1.0 / n_factors_outside_main_branch for factor_i in factors}
        elif n_factors_outside_main_branch == 0:
            factors_weights = {factor_i: 1.0 / n_factors_in_main_branch for factor_i in factors}
        else:
    ...
    def update_msg_node2factor(self, node, cur_factor, all_neighbor_factors, flag):
        ...
        if True:
            ...
            msg_node2factor = reduce(lambda x, y: x + y, list(map(lambda factor_i: (
                    self.get_factor_weight(node, factor_i) * factors_weights[factor_i] * self._msg_factors2nodes[
                (factor_i, node)]), all_neighbor_factors)) + [0])
            ...
            msg_node2factor = (1.0 - beta_1) * msg_node2factor + beta_1 * float(self.belief[node].values[0])
    ...
    def update_belief(self):
        for node_i in self.belief.columns.values:
            ...
            belief_new = reduce(lambda x, y: x + y, list(map(lambda factor_i: (
                    self.get_factor_weight(node_i, factor_i) * factors_weights[factor_i] * self._msg_factors2nodes[
                (factor_i, node_i)]), all_neighbor_factors)) + [0])

            self.belief[node_i] = belief_new

        # if self.args.belief_normalization:
        if False:
            self.belief = self.belief.div(np.linalg.norm(self.belief, axis=1), axis=0) * (10.0**len(str(len(self.belief.columns.values))))
    ...
    def get_imbalance_loss(self):
        ...
        tmp = tmp.values
        tmp1 = (tmp / np.linalg.norm(tmp, axis=1)) * (10.0**(len(str(len(self.factors_nodes.columns)))))
    ...
    def run(self):
        ...
        for i in range(1, (self.args.n_epoch_mpo + 1)):
            ...
            for factor_i, parent_child_nodes in self.factors.items():  # update msg_factors2nodes
                parent_nodes = parent_child_nodes["parent_nodes"]  # parent and child are nodes=flux values
            ...
            for node_i, parent_child_factors in self.nodes.items():  # update msg_nodes2factors
